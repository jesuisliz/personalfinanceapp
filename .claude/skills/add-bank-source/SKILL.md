...
**Check date-range overlap for every account the new upload touches, not just the newly-onboarded one.** Confirmed real case (2026-08-05): a batch upload had two files — one for a brand-new account (`4947`, Travel Rewards) and one for an *already-configured* account (`9837`, Cash Rewards, already using the same `boa_native_card` parser for months). The new account's overlap was checked and trimmed before import; the already-configured one was assumed "fine, already handled" and skipped — but it also overlapped ~2 weeks of already-imported aggregator data and silently produced 15 duplicate transactions ($902.67 inflated spend) before being caught after the fact and deleted. "Already handled" only means the filename routes to the right parser — it says nothing about whether this file's date range was already imported by a *different* format. Check every file in a batch against the max already-imported date for its account, every time, even for accounts you've imported via this exact parser before — see the two overlap-checking gotchas added to the Step 3 list below.
...
- **BOA's aggregator export and its native per-account exports can disagree on which date the same transaction "belongs to" by a couple of days** (2026-08-05: same Olive Garden/Barnes & Noble charges, aggregator dated 07/25, native export dated 07/27 — the aggregator uses a transaction date, the native export gives only `Posted Date`) — when checking for date-range overlap, match candidate duplicates by amount first and allow a few days of date slack, not an exact date match, or you'll under-count the overlap and under-trim the file.
- **Hash dedupe catching a few rows across a format change is not proof the reimport was clean** (2026-08-05: 2 of 17 overlapping rows happened to hash-match exactly, because BOA's aggregator "Simple Description" passed those two specific merchants' raw payee text through unchanged; the other 15 didn't match and were inserted as duplicates) — if a re-imported overlapping file shows *any* rows silently deduped, treat that as a sign the date range overlaps prior data in a different format and go look for the rest, rather than assuming a non-zero dedupe count means it caught everything.
