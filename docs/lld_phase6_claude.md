...
**Status: M1-M5 built and verified (2026-07-30). Phase 6 complete.**
...
**Built and verified 2026-07-30.** Added `history.list_conversations` (ordered by `updated_at` desc) and `GET /chat/conversations` returning a new `ChatConversationOut` schema (`id`, `title`, `updated_at`). `Chat.tsx` gained a left sidebar (plain Tailwind, matching the app's existing dark theme) listing conversations with title + `formatRelativeTime` (new small helper in `format.ts`: "just now"/"Xm ago"/"Xh ago"/"Xd ago"/calendar date beyond a week) and a "New Conversation" button that clears the active thread and the `localStorage` pointer — no new create endpoint, since the next message naturally starts a fresh conversation via M1's existing "no `conversation_id` passed" path. Clicking a list entry reuses M2's `fetchConversationMessages`. The active conversation is highlighted; the list refreshes after every sent message (title/`updated_at` can change) and on mount. 2 new backend tests (144 total), tsc/oxlint/vitest clean. Live-verified in the browser: sent a message (conversation A appeared in the sidebar, "just now"), clicked "New Conversation" (view reset to empty, nothing highlighted), sent a second message (created conversation B, not appended to A — confirmed both by the UI showing 2 distinct entries and by raw SQL against `finance.db` showing 2 `chat_conversations` rows each with exactly 2 `chat_messages`), then clicked back to conversation A and confirmed its exact prior messages (including the tool-call table) reappeared. Test rows deleted from `finance.db` afterward. Backend dev server needed a restart before testing — a stray earlier instance was holding port 8000 with pre-M3 code despite `--reload` (see [[dev_server_gotchas]]).
...
**Built and verified 2026-07-30.** Before implementing, found that the design's premise — "`ChatMessage.conversation_id` cascades on delete, enforced at the ORM/DB level" (§2) — doesn't actually hold: this app's SQLite connection never sets `PRAGMA foreign_keys=ON` (checked `db.py`, confirmed no other file does either), so the `ondelete="CASCADE"` annotation on the FK column is inert. Deleting a `ChatConversation` row directly would have left its `ChatMessage` rows orphaned with dangling `conversation_id`s, not cascaded. Presented two fixes to the user: turn on `PRAGMA foreign_keys=ON` globally (matches the doc's wording exactly, but this app has 6+ other FK columns across `categories`/`accounts`/`transactions`/`transfer_match` that have never been enforced — flipping it on risks surfacing or breaking unrelated, unaudited delete/update paths elsewhere) vs. an explicit application-level delete scoped only to this one function. User chose the app-level delete (recommended option) for its zero blast radius. `history.delete_conversation` now explicitly deletes a conversation's `ChatMessage` rows before deleting the `ChatConversation` row, in one transaction — same end-user guarantee (a deleted conversation always deletes its messages too), just enforced in Python instead of relying on a DB setting this codebase doesn't use. Added `DELETE /chat/conversations/{id}` (404 for an unknown id, 204 on success, matching the existing `DELETE /goals/{id}` convention in `routers/planning.py`). `Chat.tsx` sidebar rows gained a delete button (inline SVG trash icon, not a bare "×" — an "×" reads as close/dismiss rather than delete) next to the title, gated behind `window.confirm()`; deleting the currently-open conversation clears the active view back to empty and removes the `localStorage` pointer, matching "New Conversation"'s reset path. 4 new backend tests (161 total), tsc/oxlint/vitest clean. Live-verified two ways: (1) `curl DELETE` against a throwaway conversation created via a real `POST /chat` call, confirmed via raw SQL that both the conversation and its 2 messages were gone while the 4 pre-existing real conversations and their messages were untouched, and confirmed `DELETE` on an unknown id returns 404; (2) in the actual browser Chat UI (with `window.confirm` stubbed to auto-accept via `javascript_tool`, to avoid a blocking native dialog), sent a real message creating a new conversation, clicked its delete icon, confirmed it vanished from the sidebar and the active view reverted to the empty state, then independently re-confirmed via raw SQL that no orphaned `chat_messages` rows existed anywhere in the database. This closes out Phase 6 — all 5 milestones built and verified.
