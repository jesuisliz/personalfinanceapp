...
**Status: complete** (2026-07-27), plus user-requested additions beyond the original scope: an "Uncategorized only" filter, and (2026-07-28) Category and Month filter dropdowns, on the Transactions tab; click-to-drill-down from any category in the breakdown to its underlying transactions. No dedicated LLD was written for this phase (a gap the project noted and corrected starting with Phase 4) — see memory `phase3_status` for the full design record: `backend/app/dashboard/aggregates.py` + `backend/app/routers/dashboard.py`, `frontend/src/Dashboard.tsx`. The Transactions-tab filters are pure client-side filtering in `frontend/src/App.tsx` (no backend changes) — consistent with how the existing Account/"Uncategorized only" filters already worked. **Update 2026-08-05** (`lld_income_breakdown_claude.md`): added an "Income by category" card mirroring "Spending by category" — every positive-amount transaction (excluding Transfers/CC Payment) grouped by category, including refunds landing back in an ordinary spending category, with its own click-to-drill-down and a matching `get_income_breakdown`/`get_income_transactions` chat tool pair.
...
- Where does my money come from? — confirmed 2026-08-05: "Income by category" card, independently verified via raw SQL (July 2026: $12,242.90 across 6 categories, exact match) and live via chat (`get_income_breakdown`).

### Phase 4 — Financial chatbot (OpenAI)

**Status: complete and live-validated with a real `OPENAI_API_KEY`** (2026-07-27). Full design in `lld_phase4_claude.md`.

Success criteria:
- You can ask a natural-language question ("How much did I spend eating out?") and get a correct answer. — confirmed live: "Dining & Drinks last month" → $1,579.68 and "Groceries last month" → $910.23, both independently verified via raw SQL (exact match). All 6 tools exercised live through the actual Chat tab, not just unit tests (116/116 backend tests passing).
- Every number in a chatbot answer was computed by backend code, never invented or estimated by the model — confirmed, and live testing caught a real gap in how well that held up in practice: the model correctly narrated the *tool-computed* number but, in a multi-month narration, mis-converted or mis-transcribed it in prose for a few entries (see `lld_phase4_claude.md` §6 for both fixes and the one residual, unfixable-by-architecture narration-fidelity limitation). The underlying tool call and its rendered table (shown alongside every reply) are always exactly correct regardless — that display-both-together decision made from the start is what makes the residual risk visible rather than hidden.
- The chatbot correctly declines or asks for clarification when a question is ambiguous, rather than guessing which numbers to use. — confirmed live for the two real "no" states this app can produce: no savings goals configured, and no current balance set — both correctly reported as such rather than fabricating a number. An unrecognized category name also still produces a recoverable tool error rather than a crash (unit-tested).

### Phase 5 — Savings goals, vacation planner, runway, scenario analysis

**Status: complete and independently verified** (2026-07-27; built before Phase 4's live validation was finished — Phase 5 never depended on the chatbot working, only on the dashboard's aggregates, which were already complete, and Phase 4's live validation was completed later the same day). Full design in `lld_phase5_claude.md`. First new persisted data since Phase 1 (`SavingsGoal`, `CurrentBalance`) — everything else so far had been pure aggregation over transactions.

Note: Phase 5's own chatbot tools (`get_savings_goals`, `get_financial_runway`) were unit-tested with a fake client when this phase was built, and were live-verified as part of Phase 4's later live-validation pass (both correctly returned the honest "not configured" states against the real database, rather than fabricating numbers).
