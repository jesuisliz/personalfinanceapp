...
class MerchantRule(Base):
    ...
    account_id: int | None (FK accounts.id, nullable)  # added 2026-07-29, see §15
...
2. `merchant_rules` scoped to the transaction's `account_id` — substring match against `description` (case-insensitive), first match wins (§15).
3. `merchant_rules` with `account_id IS NULL` (institution/global-wide) — same substring match, checked only if no account-scoped rule matched.
4. Otherwise display the raw `description` as-is.

Sets `clean_description` and, if the winning rule specifies one, `category_id` (only if `category_id` is still `NULL`). Account-scoped rules exist because some bank statement text (e.g. BofA's generic `"PAYMENT - THANK YOU"`) carries no card-identifying info at all — a global rule matching that text would risk mislabeling a different account's payment that happens to reuse the same generic phrasing (§15 found a live instance of exactly this, already bleeding a Cash-Rewards-card-shaped label onto two American Express transactions).
...
## 15. 2026-07-29, later still — Best Buy miscategorization, then account-scoped `MerchantRule` added after a BofA Cash Rewards labeling gap

**Best Buy → Fees & Adjustments instead of Credit Card Payment (Jan-Apr 2026).** User report: "Best Buy payments only showing up for May, June and July." Same dual-source-format shape as §14's CMP bug, different specifics: Jan-Apr came from the old, category-less BOA checking-statement format (`Date, Description, Amount, Running Bal.` — no raw-category column at all, confirmed by inspecting the actual source CSV rows), so `raw_category` was `NULL` and no existing merchant rule recognized the raw ACH descriptor (`BEST BUY DES:AUTO PYMT ID:... CITIAUTFDR WEB`). With `raw_category NULL` and no merchant match, these 5 transactions should have stayed *uncategorized* under the resolution order in §4 — instead they carried `category_id` for **Fees & Adjustments**, which no rule in the codebase produces from a `NULL raw_category`. Conclusion: a manual category edit, made before this session, most likely because the cryptic ACH text read like a bank fee before anyone connected it to the Best Buy card. Fix: added merchant rule `"BEST BUY DES:"` → `Credit Card Payment` (global — safe, since no other transaction anywhere in the data contains "best buy" outside this one payment pattern), then corrected the 5 existing rows via `PATCH /transactions/{id}` (manual edits are never auto-overwritten, so the rule alone doesn't fix history). Full suite 134/134 (was 133 after §14) after adding this and the fix below.

**`MerchantRule` gained account scoping, prompted by a follow-up ask about the BofA Cash Rewards card.** User then asked to isolate all Credit Card Payment rows for the Cash Rewards card via a single Merchant-search query. Investigation found the category was already correct for every month except a genuinely missing April import (no transactions at all that month on that account — a data gap, not a bug) — but only 2 of 6 payment rows had a `clean_description` set, because the other 4 used generic bank statement text (`"PAYMENT - THANK YOU"`, `"Credit Card Payment Received"`) with no card-identifying substring, so a naive global merchant rule risked mislabeling a *different* BofA card's payment if it ever produced the same generic text. This risk turned out to already be live: a pre-existing global rule (id 53, `"PAYMENT - THANK YOU"` → `"Credit Card Payment Received"`, `category_id=Transfers`, predating this session) was already applied to 2 American Express `"AUTOPAY PAYMENT - THANK YOU"` transactions, giving them a Cash-Rewards-shaped generic label.

Rather than accept that risk for the new rule, added `account_id` to `MerchantRule` (mirroring `CategoryRule`'s existing pattern from §12) and updated `apply_merchant_rules` to check account-scoped rules before global ones (§5). Added two rules scoped to the Cash Rewards account: `"PAYMENT - THANK YOU"` and `"Credit Card Payment Received"` → `"BofA Cash Rewards Card Payment"`. The 3 Jan-Mar rows already labeled by rule 53 needed a direct `PATCH` (existing `clean_description` is never auto-overwritten, same protection as always) since the new rule alone can't retroactively relabel them. Left rule 53 and the 2 mislabeled Amex rows untouched — a real pre-existing gap, flagged to the user, but out of scope for this fix (a decision point, not a silent leave-behind).

Required an **existing SQLite table's schema to change** for the first time since Phase 1/5 — `Base.metadata.create_all()` doesn't alter existing tables, so `merchant_rules.account_id` had to be added to the live `finance.db` via a manual `ALTER TABLE`. Also hit a new (7th) instance of the dev-server reload gotchas: `--reload` logged `WatchFiles detected changes in 'app\models.py'. Reloading...` but then never logged a matching "Application startup complete" or error — the server kept responding 200 the whole time, but on the *old* schema (confirmed via `GET /merchant-rules` missing the new `account_id` field). Full process-tree kill + restart fixed it — see [[dev_server_gotchas]] for the general pattern. Regression test `test_account_scoped_merchant_rule_overrides_global_rule_for_same_text` added to `test_categories.py`; full suite 134/134. Verified live: Merchant search `"cash rewards"` alone (no account/category filter) now returns all 6 real Cash Rewards payments.
